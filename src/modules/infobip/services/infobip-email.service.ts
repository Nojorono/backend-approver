...
import { INFOBIP_ENDPOINTS } from '../../../config/infobip.config';
...
export class InfobipEmailService {
  ...
  async sendEmail(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        this.logger.log(`Sending email to ${emailData.to.length} recipient(s) - Attempt ${attempt}/${maxRetries}`);
        
        // Validate required fields
        if (!emailData.to || emailData.to.length === 0) {
          throw new BadRequestException('Missing required fields: to is required');
        }
        
        if (!emailData.html && !emailData.text) {
          throw new BadRequestException('Missing required fields: either html or text content is required');
        }
        ...
        emailData.to.forEach((recipient, index) => {
          if (!recipient.email) {
            throw new BadRequestException(`Missing email address for recipient at index ${index}`);
          }
        ...
        if (emailData.cc) {
          emailData.cc.forEach((recipient, index) => {
            if (!recipient.email) {
              throw new BadRequestException(`Missing email address for CC recipient at index ${index}`);
            }
        ...
        if (emailData.bcc) {
          emailData.bcc.forEach((recipient, index) => {
            if (!recipient.email) {
              throw new BadRequestException(`Missing email address for BCC recipient at index ${index}`);
            }
        ...
        if (emailData.attachments) {
          emailData.attachments.forEach((attachment, index) => {
            formData.append(`attachment${index}`, Buffer.from(attachment.content, 'base64'), {
        ...
        if (response.data.messages && response.data.messages.length > 0) {
          ...
          if (messageStatus.groupId === 1 || messageStatus.groupId === 2) {
            this.logger.log(`Email sent successfully. Message ID: ${response.data.messages[0]?.messageId}, Status: ${messageStatus.name}`);
            return response.data;
          } else {
            throw new BadRequestException(`Email delivery failed: ${messageStatus.description}`);
          }
        } else {
        ...
        lastError = error;
        this.logger.error(`Failed to send email (Attempt ${attempt}/${maxRetries}):`, {
          error: error.message,
  ...
  async getEmailReports(
    ...
    try {
      ...
      const response = await client.get<{ results: EmailReportDto[]; totalCount: number }>(
        `${INFOBIP_ENDPOINTS.EMAIL.REPORTS}?${params.toString()}`
      );

      this.logger.log(`Retrieved ${response.data.results.length} email reports`);
      return response.data;
    } catch (error) {
  ...
  async getEmailReportById(
    ...
    try {
      ...
      this.logger.error(`Failed to fetch email report for message ID ${messageId}:`, error);
      throw new BadRequestException(
        error.response?.data?.requestError?.serviceException?.text || 
  ...
  async getEmailTemplates(
    ...
    try {
      ...
      this.logger.log(`Retrieved ${response.data?.length || 0} email templates`);
      return response.data;
    } catch (error) {
    ...
    try {
      ...
      for (const emailData of emails) {
        try {
          ...
          this.logger.error(`Failed to send email to ${emailData.to[0]?.email}:`, error);
          results.push({
            messages: [{
              messageId: '',
              status: {
      ...
      this.logger.log(`Bulk email sending completed. ${results.length} emails processed`);
      return results;
    } catch (error) {
